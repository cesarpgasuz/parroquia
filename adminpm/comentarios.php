...
$consulta = "SELECT * FROM comentarios order by fecha asc";
...
$totalRows_comentarios = $resultado -> num_rows;
...
if (isset($_POST['buscar']) and $_POST['buscar']!="") {
  extract($_POST);
  $consulta = "SELECT * from comentarios where nombre like '%$bcampo%' or comentario like '%$bcampo%'";
  if (!$resultado = $conexion -> query($consulta)) {
    echo "Lo sentimos, este sitio web está experimentando problemas.";
    echo "Error: la ejecucion de la consulta fallo debido a \n";
    echo "Query: ".$consulta."\n";
...
<h1 style="margin-left: 30px;">
  Comentarios (<?php echo $totalRows_comentarios; ?>) 
</h1>
        ...
        <div class="col-xs-12">
           ...
           <tbody>
            ...
            function b(clave){
					    ...
					    if(v==true){
					      location.href="comentarios.php?Eliminar=1&ID="+clave;
					      }
					  }
           ...
           while ($row_comentarios = $resultado -> fetch_assoc()){ ?>

 					  <tr>
					    <td style="font-weight:bold;">
					    <?php echo $row_comentarios['fecha']; ?></td>
					    <td class="normal">
					    <?php echo iconv("ISO-8859-1","UTF-8",$row_comentarios['nombre']); ?></td>
              <td class="normal">
              <?php echo iconv("ISO-8859-1","UTF-8",$row_comentarios['comentario']); ?></td>
              <td>
  				      <div class="myboton btn btn-danger">
  					      <a onclick="b('<?php echo $row_comentarios['id']; ?>')" href="#">
  					      Eliminar
  					      </a>
  					      </div>
					    </td>
